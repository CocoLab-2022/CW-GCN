...
parser.add_argument('--lr', type=float, default=3e-2,help='3e-2')
parser.add_argument('--cuda', type=str, default="2")
parser.add_argument('--n_epoch', type=int, default=2000)
parser.add_argument('--hidden', type=int, default=128,
                    help='128Number of hidden units.')
parser.add_argument('--gfeat', type=int, default=16,
                    help='16Number of hidden units.')
parser.add_argument('--nfeat', type=float, default=8189,
                    help='Dropout rate (1 - keep probability).')
parser.add_argument('--dropout', type=float, default=0.5,
                    help='Dropout rate (1 - keep probability).')
parser.add_argument('--data_src', type=str, default='Blog1',
                    help='source dataset name')
parser.add_argument('--data_trg', type=str, default='Blog2',
                    help='target dataset name')
parser.add_argument('--classes', type=int, default=6,
                    help='classes number')
parser.add_argument('--model_path', type=str, default='models')
parser.add_argument('--lambda_d', type=float, default=0.5,
                    help='0.5hyperparameter for domain loss')
parser.add_argument('--lambda_r', type=float, default=1,
                    help='1hyperparameter for reconstruction loss')
parser.add_argument('--lambda_f', type=float, default=0.0001,
                    help='0.0001hyperparameter for different loss')
...
''' Load data '''
adj_s, features_s, labels_s, idx_s,X_n_s,row_adj = load_data_citation_noisy(dataset=args.data_src)
adj_t, features_t, labels_t, idx_t,X_n_t = load_data_citation(dataset=args.data_trg)

''' Load adj labels for reconstruction '''
adj_label_s,pos_weight_s,norm_s = load_adj_label_for_reconstruction_noisy(args.data_src,row_adj)
# adj_label_s,pos_weight_s,norm_s = load_adj_label_for_reconstruction(args.data_src)
...
def test(feature,adj,ppmi,label):
    ...
    print("preds",preds)
    print("labels",labels)
    macro = f1_score(preds.detach().cpu().numpy(), labels.detach().cpu().numpy(), average='macro')
    micro = f1_score(preds.detach().cpu().numpy(), labels.detach().cpu().numpy(), average='micro')
...
for epoch in range(args.n_epoch):
    ...
    if (epoch+1)%10 == 0:
        acc_trg,macro,micro = test(features_t,adj_t,X_n_t,labels_t)
        if acc_trg > best_acc:
            best_acc = acc_trg
        if macro > best_macro:
            best_macro = macro
        ...
        if epoch >= 1909:
            avg_acc.append(acc_trg.tolist()) #noted by hhy
            avg_macro.append(macro.tolist()) #noted by hhy
            avg_micro.append(micro.tolist()) #noted by hhy
print('best acc :{}'.format(best_acc))
print('best macro :{}'.format(best_macro))
print('best micro :{}'.format(best_micro))
print("avg_acc",avg_acc)
print("last 10 average acc:",np.mean(avg_acc))
print("last 10 average macro:",np.mean(avg_macro))
print("last 10 average micro:",np.mean(avg_micro))
print('done')
