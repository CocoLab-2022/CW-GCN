...
def load_data_citation_noisy(path="data/",dataset="citation"):
    file = str(path) + str(dataset)
    net = sio.loadmat(file)
    features, adj, labels = net['attrb'], net['network'], net['group']
    if not isinstance(features, scipy.sparse.lil_matrix):
    ...
    selected_idx = np.random.choice(len(labels), int(0.1* len(labels)), replace=False)
    ...
    selected_idx_2 = np.random.choice(len(labels), int(0. * len(labels)), replace=False)
    # selected_idx_2 =selected_idx
    ...
    for i in range(selected_idx_2.shape[0]):
        ...
        # selected_idx_idx = np.random.choice(permuted_idx, 6000, replace=False)
        # x=np.random.normal(scale=10,size=xs.shape[1])#1-x[:,selected_idx_idx]

        x = scipy.stats.cauchy.rvs(loc=0, scale=1, size=features.shape[1])
        # x= np.array(x).reshape(1,len(x))
        # print(x.shape)
        features[selected_idx_2[i], :] = x
    ...
    return adj, features, labels,idx_all,X_n, row_adj
    # return adj, features, labels,idx_all,X_n
...
def load_adj_label_for_reconstruction(dataset_name):
    A,_,_ = load_network(dataset=dataset_name) #default
    # _, _, _, _,_,A = load_data_citation(dataset=dataset_name)
...
def load_adj_label_for_reconstruction_noisy(dataset_name,A):
    # A,_,_ = load_network(dataset=dataset_name) #default
    # _, _, _, _,_,A = load_data_citationnoisy(dataset=dataset_name)
