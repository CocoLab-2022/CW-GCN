...
class Model(object):
    ...
    def build(self):
        ...
        self.weight = self.outputs - self.outputs
...
class GCN(Model):
    def __init__(self, placeholders, input_dim, source_num, target_num, **kwargs):
        ...
        # self.placeholders['labels_mask']
        self.embedding_1 = placeholders['embedding_1']
        self.embedding_2 = placeholders['embedding_2']
        self.mean_s = placeholders['mean_s']
        self.std_s = placeholders['std_s']
        self.mean_s_2 = placeholders['mean_s_2']
        self.std_s_2 = placeholders['std_s_2']
    ...
    def _loss(self):
        ...
        mean_s, var_s = tf.nn.moments(self.embedding_1, axes=[0])
        std_s = var_s
        std_s = tf.sqrt(std_s)
        mean_t, var_t = tf.nn.moments(self.graph_P1_0, axes=[0])
        std_t = tf.sqrt(var_t / 1 + 1e-10)
        self.loss += tf.reduce_sum(tf.square((self.mean_s - mean_t))) + tf.reduce_sum(tf.square((self.std_s - std_t)))
        mean_s_2, var_s_2 = tf.nn.moments(self.embedding_2, axes=[0])
        std_s_2 = tf.sqrt(var_s_2)

        mean_t_2, var_t_2 = tf.nn.moments(self.graph_P1, axes=[0])

        std_t_2 = tf.sqrt(var_t_2 / 1 + 1e-10)

        # self.loss += 0.1*(tf.reduce_sum(tf.pow((mean_s_2 - mean_t_2), 2)) + tf.reduce_sum(tf.pow((std_s_2 - std_t_2), 2)))
        # self.loss +=tf.reduce_sum(tf.pow(tf.maximum((mean_s_2- mean_t_2),1e-2),2))+tf.reduce_sum(tf.pow(tf.maximum((std_s_2- std_t_2),1e-2),2))
        z = tf.maximum((mean_s_2 - mean_t_2), 1e-3)
        x = tf.maximum((std_s_2 - std_t_2), 1e-3)

        self.var_s = std_t_2
        self.loss += tf.reduce_sum(tf.square(self.mean_s_2 - mean_t_2)) + tf.reduce_sum(
            tf.square(self.std_s_2 - std_t_2))
